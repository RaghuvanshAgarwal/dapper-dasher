...
constexpr int jump_force{900}; // pixels/frame
constexpr int gravity{2'000}; // (pixels/frame)/frame
...
public:
    spritesheet(const Rectangle* draw_rect, const int rows, const int columns, const int total_frames,
...
public:
    ...
    {
        player_ground_ = {static_cast<float>(window_height - player_height)};
        player_x_ = {static_cast<float>(window_width - player_width) * 0.25f};
        texture_ = texture;
        is_in_air_ = false;
        rect_ = new Rectangle{0, 0, static_cast<float>(player_width), static_cast<float>(player_height)};
    ...
    void on_update()
    {
        ...
        {
            velocity_ += static_cast<int>(static_cast<float>(gravity) * GetFrameTime());
            is_in_air_ = true;
        }
...
int main()
{
    ...
    UnloadTexture(nebula_spritesheet);
    CloseWindow();
}
